...
test('six-month time range always returned for historical finances', async () => {
	...
	expect(new Date(data[0]?.BucketValue || new Date()).getMonth()).toBe((new Date().getMonth() - 5) % 12);
	expect(new Date(data[1]?.BucketValue || new Date()).getMonth()).toBe((new Date().getMonth() - 4) % 12);
	expect(new Date(data[2]?.BucketValue || new Date()).getMonth()).toBe((new Date().getMonth() - 3) % 12);
	expect(new Date(data[3]?.BucketValue || new Date()).getMonth()).toBe((new Date().getMonth() - 2) % 12);
	expect(new Date(data[4]?.BucketValue || new Date()).getMonth()).toBe((new Date().getMonth() - 1) % 12);
