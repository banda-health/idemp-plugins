...
test('implementer role has correct access', async () => {
	await globalThis.__VALUE_OBJECT__.login(RoleName.Implementer);
	const reportMenuList = (
		await query(globalThis.__VALUE_OBJECT__)({
			query: Ad_MenuGetDocument,
			variables: { Size: 1, Filter: JSON.stringify({ ad_menu_uu: reportsMenuRootUuid }) },
	...
	expect(processes.find((process) => process.UU === processUuid.moh705AOutpatientUnder5YearsSummary)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.moh705BOutpatientOver5YearsSummary),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.moh705BOutpatientOver5YearsSummary)).not.toBeUndefined();
	...
	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.visitInvoice),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.visitInvoice)).not.toBeUndefined();
	...
	expect(processes.find((process) => process.UU === processUuid.voidedTransactionsList)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.inventoryQuantityReport),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.inventoryQuantityReport)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.dailyCashierCollections),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.dailyCashierCollections)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.expiredProductsList),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.expiredProductsList)).not.toBeUndefined();
	...
	expect(processes.find((process) => process.UU === processUuid.createPriceList)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.moh706LaboratoryTestSummary),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.moh706LaboratoryTestSummary)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.patients),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.patients)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.otcSales),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.otcSales)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.expenses),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.expenses)).not.toBeUndefined();

	expect(processes.find((process) => process.UU === processUuid.resetStock)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.servicesList),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.servicesList)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.incomeStatement),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.incomeStatement)).not.toBeUndefined();

	expect(processes.find((process) => process.UU === processUuid.stockTransfers)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.moh747FamilyPlanning),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.moh747FamilyPlanning)).not.toBeUndefined();

	expect(
		reportMenuList.find((reportMenu) => reportMenu.AD_Process?.UU === processUuid.deletedDraftedVisits),
	).not.toBeUndefined();
	expect(processes.find((process) => process.UU === processUuid.deletedDraftedVisits)).not.toBeUndefined();
