...
test('verify system configuration for login security settings', async () => {
	...
	expect(maxLoginAttemptConfig.Value).toBe('0');
	...
	expect(maxInactivePeriodConfig.Value).toBe('0');
});

test.skip('verify login is blocked after maximum inactive days', async () => {
...
test.skip('verify login is blocked after maximum failed attempts', async () => {
	let valueObject = globalThis.__VALUE_OBJECT__;
	await valueObject.login();
