...
/**
 * This class is meant to populate all needed data for the GraphQL API tests
 */
public class BandaGraphQLDataPopulator extends ChuBoePopulateFactoryVO {
	@IPopulateAnnotation.CanRunBeforeClass
	public void createClientForGraphQLTests() throws SQLException {
		...
		MClient_BH testClient =
				new Query(valueObject.getContext(), MClient_BH.Table_Name, MClient_BH.COLUMNNAME_Name + "=?",
						valueObject.getTransactionName()).setParameters("GraphQL Test Client").first();
		...
		if (testClient == null) {
			...
			valueObject.setProcessInformationParameters(List.of(
					new ProcessInfoParameter("ClientName", "GraphQL Test Client", null, null, null),
		...
		String sql = "SELECT '" + valueObject.getStepMessageLong() +
				"' as name, EXISTS(SELECT * FROM ad_client WHERE name = 'GraphQL Test Client') as result ";
		addAssertionSQL(sql);
		...
		try {
			...
			valueObject.setStepName("Make sure the GraphQL client always uses any new features");
			...
			valueObject.setProcessTableId(0);
			valueObject.setProcessInformationParameters(List.of(
					new ProcessInfoParameter("source", "BHLabs", null, null, null)
			));
			ChuBoeCreateEntity.runProcessAsSystem(valueObject);
			commitEx();
	...
	public void graphqlTestClientExists() {
		ChuBoePopulateVO valueObject = new ChuBoePopulateVO();
		valueObject.prepareIt(getScenarioName(), true, get_TrxName());
		assertThat("VO validation gives no errors", valueObject.getErrorMessage(), is(nullValue()));
