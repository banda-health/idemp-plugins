...
bhDefine(
	['helper/util', 'config/classNames', 'helper/templateManager'],
	function (util, classNames, templateManager) {
		let self = {};
		...
		function addLoadingDisplay() {
			...
			loadingDiv.innerHTML = templateManager.getTemplate(
				'loadingDiv'
		...
		function initLoginScreen() {
			let advancedRowElement = templateManager.getTemplate('loginAdvancedRow')
				.firstElementChild;
			let originalSelectRoleRowElement = document.querySelector(
				'#rowSelectRole'
			);
			...
			selectRoleRowElement.innerHTML = selectRoleRowElement.innerHTML.replace(
				/((id|for)=")(\w+\-?\w+)(")/g,
				'$1$3_BH$4'
			);
			...
			selectRoleRowElement
				.querySelector('input[type="checkbox"]')
				.addEventListener('change', syncRealRoleSelectorWithFake);
			...
			function initLanguageSelectorListener() {
				...
				setInterval(
					checkIfDifferentLanguageSelected,
					languageChangedDelayCheckerMS
				);
				...
				function onLanguageChanged() {
					let realSelectRoleLabel = originalSelectRoleRowElement.querySelector(
						'.login-field .z-checkbox-content'
					);
					let fakeSelectRoleLabel = selectRoleRowElement.querySelector(
						'.login-field .z-checkbox-content'
					);
					...
					if (realSelectRoleLabel.innerHTML === fakeSelectRoleLabel.innerHTML) {
						...
						if (
							new Date() - checkStartTime <=
							maxTimeToCheckForLanguageUpdatesMS
						) {
			...
			function syncRealRoleSelectorWithFake(e) {
				...
				let iDempiereChangeRoleCheckbox = originalSelectRoleRowElement.querySelector(
					'input[type="checkbox"]'
				);
