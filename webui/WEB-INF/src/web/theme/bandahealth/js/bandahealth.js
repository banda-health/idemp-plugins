...
bhDefine(
	...
	function (util, DomObserver, classNames, translation, templateManager) {
		...
		// Pull these numbers from bandahealth.scss, lines 243-342
		const toolbarButtonPosition = {
			UNDO: 8,
			GRID_TOGGLE: 10,
		};
		...
		self.initPage = (function initPageScope() {
			...
			return function initPage() {
				...
				let ribbon = document.querySelector(
					'.z-toolbar-tabs .z-toolbar-content.z-toolbar-start'
				);
				if (!pageHasLoaded()) {
				...
				function hideEastPanel() {
					let eastPanelCollapseButton = document.querySelectorAll(
						'.desktop-layout .z-east-splitter-button i'
					)[1];
		...
		function addDomObservationMethods() {
			util.executeFunctionWhenElementPresent(
				...
				function createDetailPaneObserver() {
					let detailPaneObserver = new DomObserver(
						...
						function handleDomUpdate() {
							...
							// are we on dashboard
							let dashboardPane = document.querySelector(
								'.desktop-hometab.z-tab.z-tab-selected'
							);
							...
							if (!pageLoadedIsLookupWindow()) {
								...
								let lookupPanelOkButton = document.querySelector(
									'button[title="OK"].img-btn.btn-ok.z-button'
								);
			...
			function areAnyTabsVisisble() {
				let tabs = document.querySelectorAll(
					'.adwindow-detailpane-tabbox .z-tabs-content li'
				);
				if (tabs.length === 0) {
					return false;
				}
				for (let i = 0; i < tabs.length; i++) {
					...
					// visible
					if (
						tabs[i].classList.contains('z-tab-selected') &&
						util.elementIsVisible(tabs[i])
					) {
						return true;
					}
				}
			...
			function areCreatingOrEditingAnEntity() {
				let entityCancelButton = getElementInCurrentWindow(
					`.adwindow-toolbar.z-toolbar-start > a:nth-child(${toolbarButtonPosition.UNDO})`
				);
				return util.elementIsVisible(entityCancelButton);
			}

			function areViewingTheStockTakePage() {
				let lastTabText = document.querySelector(
					'.desktop-tabbox .z-tabs .z-tabs-content li.z-tab-selected .z-tab-text'
				);
				return (
					lastTabText && (lastTabText.innerText || '').includes('Stock Take')
				);
			}
			...
			function pageLoadedIsLookupWindow() {
				...
				let windowHeader = document.querySelector(
					'div .z-tabs ul li:nth-child(3) a span'
				);
				if (windowHeader != null) {
					let isFromDashboardCheck = windowHeader.textContent.match(
						/^\s..\s\w{6}[a-z]$/i
					);
		...
		function addTabCountResetObservation() {
			let mainTab = document.querySelector(
				'.desktop-tabbox.z-tabbox.z-tabbox-top > .z-tabs > .z-tabs-content'
			);
		...
		function addDashboardPanelMethods() {
			util.executeFunctionWhenElementPresent(
				...
				function createDashboardObserver() {
					let dashBoardObserver = new DomObserver(
						document.querySelector('.desktop-tabbox'),
						function handleDomUpdate() {
							let bodyTag = document.body;
		...
		function closeAllButHomeTab() {
			if (getNumberOfIDempTabsOpen() > 1) {
				let tabsToClose = document.querySelectorAll(
					'.desktop-tabbox .z-tabs .z-tabs-content li:not(:first-child):not(:nth-child(2)) .z-tab-button i'
				);
		...
		function closeTabDetailPane() {
			let closeTabDetailPaneButton = document.querySelector(
		...
		function getDesktopHeaderPopupAndExecuteFunction(functionToExecute) {
			let idempTableFetchButton = document.querySelector(
				'.z-toolbar-tabs .z-toolbar-content.z-toolbar-start a'
			);
		...
		function getNumberOfIDempTabsOpen() {
			let tabs = document.querySelectorAll(
				'.desktop-tabbox .z-tabs .z-tabs-content li'
			);
		...
		function handleClickNavigation(e) {
			if (
				...
				if (
					clickedSpan.parentNode.parentNode.classList.contains('z-tab-selected')
				) {
					hasHashChangedDueToClick = true;
			...
			function clickWasOnDetailPaneExpander() {
				return (
					(e.target.classList.contains('z-icon-chevron-up') &&
			...
			function userClickedDetailPaneTab() {
				...
				if (
					...
					greatGrandparent =
						(greatGrandparent.parentNode || {}).parentNode || {};
				}

				if (
					greatGrandparent.classList &&
					greatGrandparent.classList.contains('adwindow-detailpane-tabbox')
				) {
					return true;
				}
				return false;
			...
			function userClickedDetailPaneNewOrEdit() {
				...
				if (
					!targetClassList.contains('z-toolbarbutton-content') &&
				...
				if (
					grandparent.classList.contains('adwindow-detailpane-toolbar') &&
					(aTag.getAttribute('title').includes('New ') ||
						aTag.getAttribute('title').includes('Edit '))
				) {
					return true;
				}
				return false;
		...
		function initializeMobileCorrectionChecks() {
			...
			function addGridToggleButton() {
				let toolbar = document.querySelector(
					'.adwindow-toolbar.mobile .z-toolbar-content'
				);
				if (!toolbar) {
					return;
				}
				let gridToggleButton = document.querySelector(
					'.adwindow-toolbar.mobile .z-toolbar-content > a[title*="Alt+T"]'
				);
				if (gridToggleButton) {
					return;
				}
				gridToggleButton = templateManager.getTemplate('gridToggleButton')
					.firstElementChild;
				...
				// correctly
				let postItButton = document.querySelector(
					'.adwindow-toolbar.mobile .z-toolbar-content > a[title*="Post-it"]'
				);
				if (postItButton != null)
					postItButton.parentNode.insertBefore(
						gridToggleButton,
		...
		function navigateToDetailEditIfUserOnGridView() {
			let editTableCell = getElementInCurrentWindow(
				'.adwindow-layout div:nth-child(2) .adtab-content:first-child .adtab-grid tr .row-indicator-selected'
			);
			if (util.elementIsVisible(editTableCell)) {
				let gridToggle = getElementInCurrentWindow(
					`.adwindow-toolbar .z-toolbar-start > a:nth-child(${toolbarButtonPosition.GRID_TOGGLE})`
				);
				if (gridToggle) {
					gridToggle.click();
				}
		...
		function openTabDetailPane() {
			let openTabDetailPaneButton = document.querySelector(
				'.z-south-collapsed .z-icon-chevron-up'
			);
			if (
				util.elementIsVisible(openTabDetailPaneButton) &&
		...
		function disableZoomAccrossWindows() {
			let zoomableLabels = document.querySelectorAll(
				'.z-label[style="cursor: pointer; text-decoration: underline;color: #333;"]'
			);
		...
		function hideSaveWhenFinalizeOrderPresent() {
			let saveButton = document.querySelector(
				"a.toolbar-button.z-toolbarbutton[title*='Alt+S']"
			);
			let finalizeOrderButton = document.querySelector(
				"div.btn[title*='Finalize']"
			);
			if (finalizeOrderButton != null) {
				let saveNewButton = document.querySelector(
					"a.toolbar-button.z-toolbarbutton[title*='Alt+A']"
				);
				$(
					`<style>.bh.organization .adwindow-toolbar .z-toolbar-start > a:nth-child(${toolbarButtonPosition.UNDO}):after{content:' New'}</style>`
				).appendTo('head');
				...
				$(
					`<style>.bh.organization .adwindow-toolbar .z-toolbar-start > a:nth-child(${toolbarButtonPosition.UNDO}):after{content:' Save'}</style>`
				).appendTo('head');
			}
		}
		...
		function appendMobileMenuButton() {
			...
			var menuButton = document.querySelector(
				'.desktop-header.mobile.z-div > table > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td:nth-child(1) > a'
			);
			...
			var headerBar = document.querySelector(
				'.desktop-center.z-center >.z-center-body >.desktop-tabbox.z-tabbox.z-tabbox-top >.z-tabs>.z-tabs-content'
			);
			headerBar.insertBefore(menuButtonAsListItem, headerBar.childNodes[0]);
		}

		function expandDashboardMenuTabPanels() {
			let tabPanels = document.querySelectorAll(
				'.z-tabbox.z-tabbox-accordion > .z-tabpanels > .z-tabpanel'
			);
		...
		function openInitialWindow() {
			...
			if (menuItem === null) {
				menuItem = document.querySelector(
					'[title="Enter patients into the system"]'
				);
			}

			if (menuItem !== null) {
				var button = menuItem.querySelector(
					'div:nth-child(3) > table > tbody > tr'
				);
				if (button !== null) {
					button.click();
				}
		...
		function getElementInCurrentWindow() {
			...
			if (!arguments) {
				...
				activeElement = listOftargetElements.item(
					listOftargetElements.length - 1
				);
			}
