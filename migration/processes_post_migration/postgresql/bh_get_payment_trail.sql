	...
	FROM
		(
			...
			SELECT
				...
				CASE
					WHEN COALESCE(SUM(gvp.payamt), 0) - COALESCE(i.charges, 0) = 0 THEN 'Visit'
					ELSE 'Visit charges and payments' END               AS "type",
				i.non_charges                                         AS debits,
					COALESCE(SUM(gvp.payamt), 0) - COALESCE(i.charges, 0) AS credits,
				10                                                    AS sort
			FROM
				c_order o
					JOIN c_bpartner bp
						ON o.c_bpartner_id = bp.c_bpartner_id
					LEFT JOIN bh_get_visit_payments(bp.ad_client_id, '-infinity'::timestamp, 'infinity'::timestamp) gvp
						ON o.c_order_id = gvp.c_order_id
					JOIN (
					SELECT
						i.c_order_id,
