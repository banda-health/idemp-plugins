...
AS
$$
WITH visit_payments AS (SELECT c_order_id,
                               SUM(payamt) AS payamt
                        FROM c_bpartner bp
                                 JOIN bh_get_visit_payments(bp.ad_client_id, '-infinity'::timestamp,
                                                            'infinity'::timestamp) gvp
                                      ON gvp.patient_id = bp.c_bpartner_id
                        WHERE bp.c_bpartner_uu = _c_bpartner_uu
                        GROUP BY c_order_id),
     transactions AS (
         -- Sum all the payments and group them by date
         SELECT transactions.bh_visit_id,
         ...
         FROM (
                  ...
                  GROUP BY i.c_invoice_id,
                           bp.c_bpartner_id,
                           dateinvoiced,
                           i.createdby) AS transactions
         GROUP BY transactions.bh_visit_id,
                  transactions.c_payment_id,
                  transactions.createdby,
                  transactions.c_bpartner_id,
                  transactions.date,
...
-- This categorizes the payments
         ...
         FROM (SELECT transactions.bh_visit_id,
                      transactions.c_payment_id,
                      transactions.createdby,
                      transactions.c_bpartner_id,
                      transactions.date,
               ...
               GROUP BY bp.c_bpartner_id) AS orderings)
