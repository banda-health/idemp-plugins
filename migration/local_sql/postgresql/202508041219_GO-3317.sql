...
DROP FUNCTION IF EXISTS get_inventory_changes(numeric, timestamp WITHOUT TIME ZONE, timestamp WITHOUT TIME ZONE, varchar);
CREATE FUNCTION get_inventory_changes(_ad_client_id numeric,
                                      ...
                                      _m_warehouse_uu varchar DEFAULT NULL)
	RETURNS TABLE
	        (
			...
			(
				...
				FROM
					m_product p
						...
						JOIN m_locator ml
						ON t.m_locator_id = ml.m_locator_id
						JOIN m_warehouse mw
				...
				WHERE p.ad_client_id = _ad_client_id
              	AND CASE
                      WHEN _m_warehouse_uu IS NOT NULL THEN mw.m_warehouse_uu = _m_warehouse_uu
                      ELSE TRUE END
				GROUP BY
