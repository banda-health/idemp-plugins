...
VALUES ((SELECT MAX(ad_process_para_id) + 1 FROM ad_process_para), 0, 0, 'Y', '2025-07-04 11:39:20.294000', 100,
        '2025-07-04 12:11:10.752000', 100, 'Storeroom', null,
        null, (SELECT ad_process_id FROM ad_process WHERE ad_process_uu = '93d7c1bc-2885-43f4-985f-90f57a414e5f'), 30,
...
drop function bh_get_inventory_changes(numeric, timestamp, timestamp);

create function bh_get_inventory_changes(_ad_client_id numeric, _start_date timestamp without time zone DEFAULT '-infinity'::timestamp without time zone, _end_date timestamp without time zone DEFAULT 'infinity'::timestamp without time zone)
    returns TABLE(m_product_id numeric, m_attributesetinstance_id numeric, purchase_price numeric, purchase_date timestamp without time zone, sell_price numeric, cost_of_goods_sold numeric, gross_profit numeric, gain_loss numeric, opening_stock numeric, ending_stock numeric, received_stock numeric, sold_stock numeric, balanced_stock numeric, m_locator_id numeric)
    stable
    language sql
as
$$
WITH product_costs AS (SELECT pc.m_product_id,
                              pc.m_attributesetinstance_id,
                              pc.purchase_price,
                              pc.purchase_date
                       FROM get_product_costs(_ad_client_id) pc)
...
FROM (SELECT m_product_id,
      ...
      FROM (SELECT p.m_product_id,
                   ...
                   COALESCE(
                                   SUM(t.movementqty)
                                   FILTER ( WHERE
                                       t.movementdate::date + t.updated::time BETWEEN _start_date AND _end_date AND
            ...
            FROM m_product p
                     ...
                     LEFT JOIN m_transaction t
                               ON p.m_product_id = t.m_product_id AND
                                  t.m_attributesetinstance_id = pc.m_attributesetinstance_id
            ...
            WHERE p.ad_client_id = _ad_client_id
            GROUP BY p.m_product_id, pc.m_attributesetinstance_id, pc.purchase_price, pc.purchase_date, t.m_locator_id) AS p
      GROUP BY m_product_id, m_attributesetinstance_id, openingstock, endingstock, receivedstock, balancestock,
               PurchaseDate,
               PurchasePrice, (STRING_TO_ARRAY(sell_information, ',', 'null'))[2]::numeric, p.m_locator_id) AS p
WHERE endingstock > 0
   OR openingstock > 0
   OR receivedstock > 0
   OR soldstock > 0
