...
public class ProcessRepository extends BaseRepository<MProcess> {
	...
	public String generateReport(String processUuid, String reportOutputType,
			List<ProcessInfoParameter> processInfoParameters) {
		MProcess process = getByUuid(processUuid);

		if (process == null) {
			throw new AdempiereException("Could not find report");
		}

		ReportOutput reportOutput = ReportOutput.PDF;
		if (reportOutputType != null) {
			try {
				reportOutput = ReportOutput.valueOf(reportOutputType);
			} catch (Exception ex) {
				logger.warning("Incorrect report type passed in: " + reportOutputType);
				logger.warning("Outputting the report as PDF instead.");
			}
		}
		...
		Map<String, MProcessPara> processParametersByNameMap = processParameterRepository
				.getGroupsByIds(MProcessPara::getAD_Process_ID,
						...
						Collectors.toMap(MProcessPara::getName, processParameter -> processParameter));
		Map<Integer, MReference> referencesByIdMap = referenceRepository
				.getByIds(processParametersByNameMap.values().stream().map(MProcessPara::getAD_Reference_ID)
		...
		processInfoParameters.forEach(processInfoParameter -> {
			...
			MProcessPara processParameter = processParametersByNameMap.get(processInfoParameter.getParameterName());
			...
			processedInfoParameters.add(new ProcessInfoParameter(
					processInfoParameter.getParameterName(),
					parameter,
	...
	@Override
	public MProcess mapInputModelToModel(MProcess entity) {
