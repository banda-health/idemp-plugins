...
public class MBankAccount_BH extends MBankAccount {
	...
	public static MBankAccount getBankAccountMappedToRefListValue(
		...
		MBHPaymentRef paymentRef = new Query(
				...
				.first();
		if (paymentRef != null) {
			Map<String, MBHPaymentRefBankAccount> paymentRefBankAccountMap = paymentRef.getBH_PaymentRef_BankAccounts()
					.stream()
					.collect(Collectors.toMap(
							MBHPaymentRefBankAccount::getBH_PaymentRefList_Value,
							paymentRefBankAccount -> paymentRefBankAccount
					));
			if (paymentRefBankAccountMap.containsKey(refListValue)) {
				...
				MBankAccount mappedBankAccount = (MBankAccount) MTable.get(ctx, MBankAccount.Table_Name)
						.getPO(paymentRefBankAccountMap.get(refListValue).getC_BankAccount_ID(), trxName);
