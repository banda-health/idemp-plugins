...
public class X_PA_ReportLineMutation extends POMutation implements GraphQLMutationResolver {
	...
	public MReportLine_BH PA_ReportLineSave(I_PA_ReportLineInput entity, DataFetchingEnvironment environment) {
		return (MReportLine_BH) super.save((X_PA_ReportLineInput) entity, environment);
	}

	public List<MReportLine_BH> PA_ReportLineSaveMany(List<I_PA_ReportLineInput> entities, DataFetchingEnvironment environment) {
		return super.saveMany(entities.stream().map(entity -> (X_PA_ReportLineInput) entity).collect(Collectors.toList()),
				environment).stream().map(entity -> (MReportLine_BH) entity).collect(Collectors.toList());
	}
