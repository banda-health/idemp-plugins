...
public class X_M_FreightMutation extends POMutation implements GraphQLMutationResolver {
	...
	public X_M_Freight M_FreightSave(I_M_FreightInput entity, DataFetchingEnvironment environment) {
		return (X_M_Freight) super.save((X_M_FreightInput) entity, environment);
	}

	public List<X_M_Freight> M_FreightSaveMany(List<I_M_FreightInput> entities, DataFetchingEnvironment environment) {
		return super.saveMany(entities.stream().map(entity -> (X_M_FreightInput) entity).collect(Collectors.toList()),
				environment).stream().map(entity -> (X_M_Freight) entity).collect(Collectors.toList());
	}
