...
public class X_AD_UserDef_TabMutation extends POMutation implements GraphQLMutationResolver {
	...
	public MUserDefTab_BH AD_UserDef_TabSave(I_AD_UserDef_TabInput entity, DataFetchingEnvironment environment) {
		return (MUserDefTab_BH) super.save((X_AD_UserDef_TabInput) entity, environment);
	}

	public List<MUserDefTab_BH> AD_UserDef_TabSaveMany(List<I_AD_UserDef_TabInput> entities, DataFetchingEnvironment environment) {
		return super.saveMany(entities.stream().map(entity -> (X_AD_UserDef_TabInput) entity).collect(Collectors.toList()),
				environment).stream().map(entity -> (MUserDefTab_BH) entity).collect(Collectors.toList());
	}
