...
public class X_BH_TabNavBtn_TabMutation extends POMutation implements GraphQLMutationResolver {
	...
	public MTabNavBtnTab BH_TabNavBtn_TabSave(I_BH_TabNavBtn_TabInput entity, DataFetchingEnvironment environment) {
		return (MTabNavBtnTab) super.save((X_BH_TabNavBtn_TabInput) entity, environment);
	}

	public List<MTabNavBtnTab> BH_TabNavBtn_TabSaveMany(List<I_BH_TabNavBtn_TabInput> entities, DataFetchingEnvironment environment) {
		return super.saveMany(entities.stream().map(entity -> (X_BH_TabNavBtn_TabInput) entity).collect(Collectors.toList()),
				environment).stream().map(entity -> (MTabNavBtnTab) entity).collect(Collectors.toList());
	}
