...
public class X_AD_TableMutation extends POMutation implements GraphQLMutationResolver {
	...
	public MTable_BH AD_TableSave(I_AD_TableInput entity, DataFetchingEnvironment environment) {
		return (MTable_BH) super.save((X_AD_TableInput) entity, environment);
	}

	public List<MTable_BH> AD_TableSaveMany(List<I_AD_TableInput> entities, DataFetchingEnvironment environment) {
		return super.saveMany(entities.stream().map(entity -> (X_AD_TableInput) entity).collect(Collectors.toList()),
				environment).stream().map(entity -> (MTable_BH) entity).collect(Collectors.toList());
	}
