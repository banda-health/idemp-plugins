...
public class X_C_UOM_ConversionInput extends MUOMConversion implements I_C_UOM_ConversionInput {
	 ...
	 *
	 * @param UU The C_UOM_Conversion_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_C_UOM_ConversionInput(@JsonProperty("UU") String UU) {
	...
	public void setC_UOM_ToInput(ForeignEntityInput C_UOM_To) {
		...
		if (C_UOM_To != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_UOM", "C_UOM_UU=?", get_TrxName())
							.setParameters(C_UOM_To.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setC_UOM_To_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_UOM with UU " + C_UOM_To.getUU());
			}
		} else {
			this.setC_UOM_To_ID(0);
		}
	}
