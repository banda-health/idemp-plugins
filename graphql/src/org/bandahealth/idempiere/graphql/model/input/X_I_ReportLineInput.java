...
import org.bandahealth.idempiere.graphql.resolver.model.X_I_ReportLineResolver;
...
public class X_I_ReportLineInput extends X_I_ReportLine implements I_I_ReportLineInput {
	 ...
	 *
	 * @param UU The I_ReportLine_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_I_ReportLineInput(@JsonProperty("UU") String UU) {
	...
	public void setCalculationTypeInput(ForeignEntityInput CalculationType) {
		...
		if (CalculationType != null) {
			...
			if (!X_I_ReportLineResolver.CALCULATIONTYPE_UUIDS_BY_VALUE.containsValue(CalculationType.getUU())) {
	...
	public void setLineTypeInput(ForeignEntityInput LineType) {
		...
		if (LineType != null) {
			// Since an entity was passed, make sure it's in the list of acceptable values
			if (!X_I_ReportLineResolver.LINETYPE_UUIDS_BY_VALUE.containsValue(LineType.getUU())) {
	...
	public void setPA_ReportSourceInput(ForeignEntityInput PA_ReportSource) {
		...
		if (PA_ReportSource != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table PA_ReportSource with UU " + PA_ReportSource.getUU());
			}
		} else {
	...
	public void setPostingTypeInput(ForeignEntityInput PostingType) {
		...
		if (PostingType != null) {
			// Since an entity was passed, make sure it's in the list of acceptable values
			if (!X_I_ReportLineResolver.POSTINGTYPE_UUIDS_BY_VALUE.containsValue(PostingType.getUU())) {
