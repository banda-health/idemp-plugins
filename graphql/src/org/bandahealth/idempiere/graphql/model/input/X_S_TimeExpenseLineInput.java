...
public class X_S_TimeExpenseLineInput extends MTimeExpenseLine implements I_S_TimeExpenseLineInput {
	 ...
	 */
	@JsonCreator
	public X_S_TimeExpenseLineInput(@JsonProperty("ID") String ID) {
		super(null, ModelUtil.getModelResultSet(new MTimeExpenseLine(null, (ResultSet) null, null), null, Table_Name, ID),
				null);
		setID(ID);
	}
	...
	public void setS_TimeExpenseInput(ForeignEntityInput S_TimeExpense) {
		...
		if (get_ID() == 0 && S_TimeExpense != null &&
				(foreignEntity = new Query(getCtx(), "S_TimeExpense", "S_TimeExpense_UU=?", get_TrxName())
						.setParameters(S_TimeExpense.getID())
						.first()) != null && foreignEntity.get_ID() != 0) {
			super.setS_TimeExpense_ID(foreignEntity.get_ID());
		}
	}
	...
	public void setS_TimeTypeInput(ForeignEntityInput S_TimeType) {
		...
		if (S_TimeType != null &&
				(foreignEntity = new Query(getCtx(), "S_TimeType", "S_TimeType_UU=?", get_TrxName())
						.setParameters(S_TimeType.getID())
						.first()) != null && foreignEntity.get_ID() != 0) {
			super.setS_TimeType_ID(foreignEntity.get_ID());
		} else {
