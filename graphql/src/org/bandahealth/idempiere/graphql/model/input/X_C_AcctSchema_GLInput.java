...
public class X_C_AcctSchema_GLInput extends MAcctSchemaGL implements I_C_AcctSchema_GLInput {
	 ...
	 *
	 * @param UU The C_AcctSchema_GL_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_C_AcctSchema_GLInput(@JsonProperty("UU") String UU) {
		...
		if (CommitmentOffset_A != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_ValidCombination", "C_ValidCombination_UU=?", get_TrxName())
							.setParameters(CommitmentOffset_A.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setCommitmentOffset_Acct(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_ValidCombination with UU " + CommitmentOffset_A.getUU());
			}
		} else {
	...
	public void setCommitmentOffsetSales_AInput(ForeignEntityInput CommitmentOffsetSales_A) {
		...
		if (CommitmentOffsetSales_A != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_ValidCombination", "C_ValidCombination_UU=?", get_TrxName())
							.setParameters(CommitmentOffsetSales_A.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setCommitmentOffsetSales_Acct(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_ValidCombination with UU " + CommitmentOffsetSales_A.getUU());
			}
		} else {
	...
	public void setCurrencyBalancing_AInput(ForeignEntityInput CurrencyBalancing_A) {
		...
		if (CurrencyBalancing_A != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_ValidCombination", "C_ValidCombination_UU=?", get_TrxName())
							.setParameters(CurrencyBalancing_A.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setCurrencyBalancing_Acct(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_ValidCombination with UU " + CurrencyBalancing_A.getUU());
			}
		} else {
	...
	public void setPPVOffset_AInput(ForeignEntityInput PPVOffset_A) {
		...
		if (PPVOffset_A != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_ValidCombination", "C_ValidCombination_UU=?", get_TrxName())
							.setParameters(PPVOffset_A.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setPPVOffset_Acct(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_ValidCombination with UU " + PPVOffset_A.getUU());
			}
		} else {
	...
	public void setSuspenseBalancing_AInput(ForeignEntityInput SuspenseBalancing_A) {
		...
		if (SuspenseBalancing_A != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_ValidCombination", "C_ValidCombination_UU=?", get_TrxName())
							.setParameters(SuspenseBalancing_A.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setSuspenseBalancing_Acct(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_ValidCombination with UU " + SuspenseBalancing_A.getUU());
			}
		} else {
