...
public class X_AD_User_SubstituteInput extends X_AD_User_Substitute implements I_AD_User_SubstituteInput {
	 ...
	 */
	@JsonCreator
	public X_AD_User_SubstituteInput(@JsonProperty("ID") String ID) {
		super(null, ModelUtil.getModelResultSet(new X_AD_User_Substitute(null, (ResultSet) null, null), null, Table_Name, ID),
				null);
		setID(ID);
	}
	...
	public void setSubstituteInput(ForeignEntityInput Substitute) {
		...
		if (Substitute != null &&
				(foreignEntity = new Query(getCtx(), "AD_User", "AD_User_UU=?", get_TrxName())
						.setParameters(Substitute.getID())
						.first()) != null && foreignEntity.get_ID() != 0) {
			super.setSubstitute_ID(foreignEntity.get_ID());
		} else {
			super.setSubstitute_ID(0);
		}
	}
