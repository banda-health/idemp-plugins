...
public class X_C_OrderLineInput extends MOrderLine_BH implements I_C_OrderLineInput {
	 ...
	 *
	 * @param UU The C_OrderLine_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_C_OrderLineInput(@JsonProperty("UU") String UU) {
		...
		if (Link_OrderLine != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_OrderLine", "C_OrderLine_UU=?", get_TrxName())
							.setParameters(Link_OrderLine.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setLink_OrderLine_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_OrderLine with UU " + Link_OrderLine.getUU());
			}
		} else {
	...
	public void setRef_OrderLineInput(ForeignEntityInput Ref_OrderLine) {
		...
		if (Ref_OrderLine != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_OrderLine", "C_OrderLine_UU=?", get_TrxName())
							.setParameters(Ref_OrderLine.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setRef_OrderLine_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_OrderLine with UU " + Ref_OrderLine.getUU());
			}
		} else {
