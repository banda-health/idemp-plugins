...
public class X_AD_Ref_TableInput extends MRefTable implements I_AD_Ref_TableInput {
	 ...
	 *
	 * @param UU The AD_Ref_Table_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_AD_Ref_TableInput(@JsonProperty("UU") String UU) {
		...
		if (AD_Disp != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "AD_Column", "AD_Column_UU=?", get_TrxName())
							.setParameters(AD_Disp.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setAD_Display(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table AD_Column with UU " + AD_Disp.getUU());
			}
		} else {
	...
	public void setAD_Input(ForeignEntityInput AD_) {
		...
		if (AD_ != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "AD_Column", "AD_Column_UU=?", get_TrxName())
							.setParameters(AD_.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setAD_Key(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table AD_Column with UU " + AD_.getUU());
			}
		} else {
			this.setAD_Key(0);
		}
