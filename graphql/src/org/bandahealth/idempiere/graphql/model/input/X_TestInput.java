...
public class X_TestInput extends MTest implements I_TestInput {
	 ...
	 *
	 * @param UU The Test_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_TestInput(@JsonProperty("UU") String UU) {
		...
		if (Account_A != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_ValidCombination", "C_ValidCombination_UU=?", get_TrxName())
							.setParameters(Account_A.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setAccount_Acct(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_ValidCombination with UU " + Account_A.getUU());
			}
		} else {
