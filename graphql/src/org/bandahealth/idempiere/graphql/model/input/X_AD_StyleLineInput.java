...
public class X_AD_StyleLineInput extends MStyleLine implements I_AD_StyleLineInput {
	 ...
	 *
	 * @param UU The AD_StyleLine_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_AD_StyleLineInput(@JsonProperty("UU") String UU) {
		...
		if (AD_Style != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "AD_Style", "AD_Style_UU=?", get_TrxName())
							.setParameters(AD_Style.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
