...
public class X_C_RemunerationInput extends X_C_Remuneration implements I_C_RemunerationInput {
	 ...
	 *
	 * @param UUID The C_Remuneration_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_C_RemunerationInput(@JsonProperty("UUID") String UUID) {
		super(Env.getCtx(), ModelUtil.getModelResultSet(new X_C_Remuneration(null, (ResultSet) null, null),
		...
		if (RemunerationType != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(RemunerationType.getUUID()).first()) != null && foreignEntity.get_ID() != 0) {
				this.setRemunerationType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UUID " + RemunerationType.getUUID());
			}
		} else {
