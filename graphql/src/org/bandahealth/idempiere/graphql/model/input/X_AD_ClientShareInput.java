...
public class X_AD_ClientShareInput extends MClientShare implements I_AD_ClientShareInput {
	 ...
	 *
	 * @param UU The AD_ClientShare_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_AD_ClientShareInput(@JsonProperty("UU") String UU) {
		...
		if (ShareType != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(ShareType.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setShareType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + ShareType.getUU());
			}
		} else {
