...
import org.bandahealth.idempiere.graphql.resolver.model.X_M_PromotionRewardResolver;
...
public class X_M_PromotionRewardInput extends X_M_PromotionReward implements I_M_PromotionRewardInput {
	...
	public void setDistributionSortingInput(ForeignEntityInput DistributionSorting) {
		...
		if (DistributionSorting != null) {
			...
			if (!X_M_PromotionRewardResolver.DISTRIBUTIONSORTING_UUIDS_BY_VALUE.containsValue(DistributionSorting.getUU())) {
	...
	public void setRewardTypeInput(ForeignEntityInput RewardType) {
		...
		if (RewardType != null) {
			// Since an entity was passed, make sure it's in the list of acceptable values
			if (!X_M_PromotionRewardResolver.REWARDTYPE_UUIDS_BY_VALUE.containsValue(RewardType.getUU())) {
				throw new AdempiereException("The reference list UU of " + RewardType.getUU() +
						" is not in the list defined for the RewardType column");
			}
			// Now make sure it's in the DB
			MRefList_BH foreignEntity;
			if ((foreignEntity =
