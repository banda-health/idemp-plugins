...
public class X_M_InventoryInput extends MInventory_BH implements I_M_InventoryInput {
	 ...
	 */
	@JsonCreator
	public X_M_InventoryInput(@JsonProperty("ID") String ID) {
		super(null, ModelUtil.getModelResultSet(new MInventory_BH(null, (ResultSet) null, null), null, Table_Name, ID),
				null);
		setID(ID);
	}
	...
	public void setM_PerpetualInvInput(ForeignEntityInput M_PerpetualInv) {
		...
		if (get_ID() == 0 && M_PerpetualInv != null &&
				(foreignEntity = new Query(getCtx(), "M_PerpetualInv", "M_PerpetualInv_UU=?", get_TrxName())
						.setParameters(M_PerpetualInv.getID())
						.first()) != null && foreignEntity.get_ID() != 0) {
			super.setM_PerpetualInv_ID(foreignEntity.get_ID());
		}
	}
