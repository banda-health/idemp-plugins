...
import org.bandahealth.idempiere.graphql.resolver.model.X_PP_Cost_CollectorResolver;
...
public class X_PP_Cost_CollectorInput extends X_PP_Cost_Collector implements I_PP_Cost_CollectorInput {
	 ...
	 *
	 * @param UU The PP_Cost_Collector_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_PP_Cost_CollectorInput(@JsonProperty("UU") String UU) {
	...
	public void setDocActionInput(ForeignEntityInput DocAction) {
		...
		if (DocAction != null) {
			// Since an entity was passed, make sure it's in the list of acceptable values
			if (!X_PP_Cost_CollectorResolver.DOCACTION_UUIDS_BY_VALUE.containsValue(DocAction.getUU())) {
	...
	public void setDocStatusInput(ForeignEntityInput DocStatus) {
		...
		if (DocStatus != null) {
			// Since an entity was passed, make sure it's in the list of acceptable values
			if (!X_PP_Cost_CollectorResolver.DOCSTATUS_UUIDS_BY_VALUE.containsValue(DocStatus.getUU())) {
	...
	public void setPP_Order_BOMLineInput(ForeignEntityInput PP_Order_BOMLine) {
		...
		if (PP_Order_BOMLine != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table PP_Order_BOMLine with UU " + PP_Order_BOMLine.getUU());
			}
		} else {
	...
	public void setReversalInput(ForeignEntityInput Reversal) {
		...
		if (Reversal != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table PP_Cost_Collector with UU " + Reversal.getUU());
			}
		} else {
	...
	public void setUser1Input(ForeignEntityInput User1) {
		...
		if (User1 != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table AD_User with UU " + User1.getUU());
			}
		} else {
			this.setUser1_ID(0);
		}
	...
	public void setUser2Input(ForeignEntityInput User2) {
		...
		if (User2 != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table AD_User with UU " + User2.getUU());
			}
		} else {
			this.setUser2_ID(0);
		}
