...
public class X_C_BankStatementLineInput extends MBankStatementLine implements I_C_BankStatementLineInput {
	 ...
	 *
	 * @param UU The C_BankStatementLine_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_C_BankStatementLineInput(@JsonProperty("UU") String UU) {
		...
		if (C_BankStatement != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_BankStatement", "C_BankStatement_UU=?", get_TrxName())
							.setParameters(C_BankStatement.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
