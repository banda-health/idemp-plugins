...
public class X_M_InOutLineInput extends MInOutLine implements I_M_InOutLineInput {
	 ...
	 *
	 * @param UU The M_InOutLine_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_M_InOutLineInput(@JsonProperty("UU") String UU) {
		...
		if (M_RMALine != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "M_RMALine", "M_RMALine_UU=?", get_TrxName())
							.setParameters(M_RMALine.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
	...
	public void setReversalLineInput(ForeignEntityInput ReversalLine) {
		...
		if (ReversalLine != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table M_InOutLine with UU " + ReversalLine.getUU());
			}
		} else {
