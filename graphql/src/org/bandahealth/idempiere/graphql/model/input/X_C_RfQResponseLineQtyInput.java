...
public class X_C_RfQResponseLineQtyInput extends MRfQResponseLineQty implements I_C_RfQResponseLineQtyInput {
	 ...
	 *
	 * @param UU The C_RfQResponseLineQty_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_C_RfQResponseLineQtyInput(@JsonProperty("UU") String UU) {
		...
		if (C_RfQLineQty != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_RfQLineQty", "C_RfQLineQty_UU=?", get_TrxName())
							.setParameters(C_RfQLineQty.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setC_RfQLineQty_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_RfQLineQty with UU " + C_RfQLineQty.getUU());
			}
		} else {
			this.setC_RfQLineQty_ID(0);
		}
	}
		...
		if (C_RfQResponseLine != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_RfQResponseLine", "C_RfQResponseLine_UU=?", get_TrxName())
							.setParameters(C_RfQResponseLine.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setC_RfQResponseLine_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_RfQResponseLine with UU " + C_RfQResponseLine.getUU());
			}
		} else {
			this.setC_RfQResponseLine_ID(0);
		}
	}
