...
public class X_AD_ReferenceInput extends MReference_BH implements I_AD_ReferenceInput {
	 ...
	 *
	 * @param UU The AD_Reference_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_AD_ReferenceInput(@JsonProperty("UU") String UU) {
		...
		if (ShowInactive != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(ShowInactive.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setShowInactive(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + ShowInactive.getUU());
			}
		} else {
	...
	public void setValidationTypeInput(ForeignEntityInput ValidationType) {
		...
		if (ValidationType != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(ValidationType.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setValidationType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + ValidationType.getUU());
			}
		} else {
