...
public class X_AD_AlertRecipientInput extends MAlertRecipient implements I_AD_AlertRecipientInput {
	 ...
	 *
	 * @param UU The AD_AlertRecipient_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_AD_AlertRecipientInput(@JsonProperty("UU") String UU) {
		...
		if (AD_Alert != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "AD_Alert", "AD_Alert_UU=?", get_TrxName())
							.setParameters(AD_Alert.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
