...
import org.bandahealth.idempiere.graphql.resolver.model.X_S_TimeExpenseResolver;
...
public class X_S_TimeExpenseInput extends MTimeExpense implements I_S_TimeExpenseInput {
	 ...
	 *
	 * @param UU The S_TimeExpense_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_S_TimeExpenseInput(@JsonProperty("UU") String UU) {
	...
	public void setDocActionInput(ForeignEntityInput DocAction) {
		...
		if (DocAction != null) {
			...
			if (!X_S_TimeExpenseResolver.DOCACTION_UUIDS_BY_VALUE.containsValue(DocAction.getUU())) {
	...
	public void setDocStatusInput(ForeignEntityInput DocStatus) {
		...
		if (DocStatus != null) {
			// Since an entity was passed, make sure it's in the list of acceptable values
			if (!X_S_TimeExpenseResolver.DOCSTATUS_UUIDS_BY_VALUE.containsValue(DocStatus.getUU())) {
	 ...
	 */
	@JsonProperty("S_TimeExpense_ID")
	public void setS_TimeExpense_IDFromJson(int S_TimeExpense_ID) {
		if (get_ID() == 0) {
			super.setS_TimeExpense_ID(S_TimeExpense_ID);
		}
	}
