...
import org.bandahealth.idempiere.graphql.resolver.model.X_M_RelatedProductResolver;
...
public class X_M_RelatedProductInput extends X_M_RelatedProduct implements I_M_RelatedProductInput {
	 ...
	 *
	 * @param UU The M_RelatedProduct_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_M_RelatedProductInput(@JsonProperty("UU") String UU) {
	...
	public void setRelatedProductInput(ForeignEntityInput RelatedProduct) {
		...
		if (RelatedProduct != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table M_Product with UU " + RelatedProduct.getUU());
			}
		} else {
	...
	public void setRelatedProductTypeInput(ForeignEntityInput RelatedProductType) {
		...
		if (RelatedProductType != null) {
			...
			if (!X_M_RelatedProductResolver.RELATEDPRODUCTTYPE_UUIDS_BY_VALUE.containsValue(RelatedProductType.getUU())) {
				throw new AdempiereException("The reference list UU of " + RelatedProductType.getUU() +
						" is not in the list defined for the RelatedProductType column");
			}
			// Now make sure it's in the DB
			MRefList_BH foreignEntity;
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(RelatedProductType.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setRelatedProductType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + RelatedProductType.getUU());
			}
		} else {
