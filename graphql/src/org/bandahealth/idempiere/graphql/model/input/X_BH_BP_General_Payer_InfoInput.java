...
public class X_BH_BP_General_Payer_InfoInput extends MBHBPGeneralPayerInfo implements I_BH_BP_General_Payer_InfoInput {
	 ...
	 *
	 * @param UU The BH_BP_General_Payer_Info_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_BH_BP_General_Payer_InfoInput(@JsonProperty("UU") String UU) {
	 ...
	 */
	@JsonProperty("BH_BP_General_Payer_Info_ID")
	public void setBH_BP_General_Payer_Info_IDFromJson(int BH_BP_General_Payer_Info_ID) {
		if (get_ID() == 0) {
	...
	public void setUU(String UU) {
		setBH_BP_General_Payer_Info_UU(UU);
	}
		...
		if (BH_BP_Payer_Info != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "BH_BP_Payer_Info", "BH_BP_Payer_Info_UU=?", get_TrxName())
							.setParameters(BH_BP_Payer_Info.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setBH_BP_Payer_Info_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table BH_BP_Payer_Info with UU " + BH_BP_Payer_Info.getUU());
			}
		} else {
