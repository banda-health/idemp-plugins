...
public class X_C_ElementValueInput extends MElementValue implements I_C_ElementValueInput {
	 ...
	 *
	 * @param UU The C_ElementValue_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_C_ElementValueInput(@JsonProperty("UU") String UU) {
	...
	public void setBPartnerTypeInput(ForeignEntityInput BPartnerType) {
		...
		if (BPartnerType != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(BPartnerType.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setBPartnerType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + BPartnerType.getUU());
			}
		} else {
