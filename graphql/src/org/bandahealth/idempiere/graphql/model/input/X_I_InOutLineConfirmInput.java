...
public class X_I_InOutLineConfirmInput extends X_I_InOutLineConfirm implements I_I_InOutLineConfirmInput {
	 ...
	 */
	@JsonCreator
	public X_I_InOutLineConfirmInput(@JsonProperty("ID") String ID) {
		super(null, ModelUtil.getModelResultSet(new X_I_InOutLineConfirm(null, (ResultSet) null, null), null, Table_Name, ID),
				null);
		setID(ID);
	}
	...
	public void setM_InOutLineConfirmInput(ForeignEntityInput M_InOutLineConfirm) {
		...
		if (M_InOutLineConfirm != null &&
				(foreignEntity = new Query(getCtx(), "M_InOutLineConfirm", "M_InOutLineConfirm_UU=?", get_TrxName())
						.setParameters(M_InOutLineConfirm.getID())
						.first()) != null && foreignEntity.get_ID() != 0) {
			super.setM_InOutLineConfirm_ID(foreignEntity.get_ID());
		} else {
