...
public class X_AD_FieldGroupInput extends MFieldGroup_BH implements I_AD_FieldGroupInput {
	 ...
	 *
	 * @param UU The AD_FieldGroup_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_AD_FieldGroupInput(@JsonProperty("UU") String UU) {
		...
		if (FieldGroupType != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(FieldGroupType.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setFieldGroupType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + FieldGroupType.getUU());
			}
		} else {
