...
public class X_A_Asset_ChangeInput extends MAssetChange implements I_A_Asset_ChangeInput {
	 ...
	 *
	 * @param UU The A_Asset_Change_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_A_Asset_ChangeInput(@JsonProperty("UU") String UU) {
		...
		if (A_Asset_Retirement != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "A_Asset_Retirement", "A_Asset_Retirement_UU=?", get_TrxName())
							.setParameters(A_Asset_Retirement.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setA_Asset_Retirement_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table A_Asset_Retirement with UU " + A_Asset_Retirement.getUU());
			}
		} else {
