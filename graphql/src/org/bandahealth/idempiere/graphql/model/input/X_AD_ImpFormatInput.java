...
public class X_AD_ImpFormatInput extends X_AD_ImpFormat implements I_AD_ImpFormatInput {
	 ...
	 *
	 * @param UUID The AD_ImpFormat_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_AD_ImpFormatInput(@JsonProperty("UUID") String UUID) {
		super(Env.getCtx(), ModelUtil.getModelResultSet(new X_AD_ImpFormat(null, (ResultSet) null, null),
		...
		if (FormatType != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(FormatType.getUUID()).first()) != null && foreignEntity.get_ID() != 0) {
				this.setFormatType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UUID " + FormatType.getUUID());
			}
		} else {
