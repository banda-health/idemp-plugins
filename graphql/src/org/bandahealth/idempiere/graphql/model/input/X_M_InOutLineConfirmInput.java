...
public class X_M_InOutLineConfirmInput extends MInOutLineConfirm implements I_M_InOutLineConfirmInput {
	 ...
	 */
	@JsonCreator
	public X_M_InOutLineConfirmInput(@JsonProperty("ID") String ID) {
		super(null, ModelUtil.getModelResultSet(new MInOutLineConfirm(null, (ResultSet) null, null), null, Table_Name, ID),
				null);
		setID(ID);
	}
	...
	public void setM_InOutConfirmInput(ForeignEntityInput M_InOutConfirm) {
		...
		if (get_ID() == 0 && M_InOutConfirm != null &&
				(foreignEntity = new Query(getCtx(), "M_InOutConfirm", "M_InOutConfirm_UU=?", get_TrxName())
						.setParameters(M_InOutConfirm.getID())
						.first()) != null && foreignEntity.get_ID() != 0) {
			super.setM_InOutConfirm_ID(foreignEntity.get_ID());
		}
	}
