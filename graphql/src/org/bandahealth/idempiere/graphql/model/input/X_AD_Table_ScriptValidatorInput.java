...
import org.bandahealth.idempiere.graphql.resolver.model.X_AD_Table_ScriptValidatorResolver;
...
public class X_AD_Table_ScriptValidatorInput extends MTableScriptValidator implements I_AD_Table_ScriptValidatorInput {
	...
	public void setEventModelValidatorInput(ForeignEntityInput EventModelValidator) {
		...
		if (EventModelValidator != null) {
			...
			if (!X_AD_Table_ScriptValidatorResolver.EVENTMODELVALIDATOR_UUIDS_BY_VALUE.containsValue(EventModelValidator.getUU())) {
				throw new AdempiereException("The reference list UU of " + EventModelValidator.getUU() +
						" is not in the list defined for the EventModelValidator column");
			}
