...
public class X_C_CashPlanLineInput extends MCashPlanLine implements I_C_CashPlanLineInput {
	 ...
	 */
	@JsonCreator
	public X_C_CashPlanLineInput(@JsonProperty("UUID") String UUID) {
		super(Env.getCtx(), ModelUtil.getModelResultSet(new MCashPlanLine(null, (ResultSet) null, null),
	...
	public void setC_CashPlanInput(ForeignEntityInput C_CashPlan) {
		...
		if (C_CashPlan != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_CashPlan", "C_CashPlan_UU=?", get_TrxName())
							.setParameters(C_CashPlan.getUUID()).first()) != null && foreignEntity.get_ID() != 0) {
				this.setC_CashPlan_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_CashPlan with UUID " + C_CashPlan.getUUID());
			}
		} else {
			this.setC_CashPlan_ID(0);
		}
	}
