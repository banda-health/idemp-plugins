...
public class X_C_CashPlanLineInput extends MCashPlanLine implements I_C_CashPlanLineInput {
	 ...
	 *
	 * @param UU The C_CashPlanLine_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_C_CashPlanLineInput(@JsonProperty("UU") String UU) {
		...
		if (C_CashPlan != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_CashPlan", "C_CashPlan_UU=?", get_TrxName())
							.setParameters(C_CashPlan.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setC_CashPlan_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_CashPlan with UU " + C_CashPlan.getUU());
			}
		} else {
