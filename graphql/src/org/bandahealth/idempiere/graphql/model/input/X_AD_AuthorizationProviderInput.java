...
public class X_AD_AuthorizationProviderInput extends MAuthorizationProvider implements I_AD_AuthorizationProviderInput {
	 ...
	 */
	@JsonCreator
	public X_AD_AuthorizationProviderInput(@JsonProperty("UUID") String UUID) {
	...
	public void setUUID(String UUID) {
		setAD_AuthorizationProvider_UU(UUID);
	}
		...
		if (AD_AuthorizationType != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(AD_AuthorizationType.getUUID()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setAD_AuthorizationType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UUID " + AD_AuthorizationType.getUUID());
			}
		} else {
