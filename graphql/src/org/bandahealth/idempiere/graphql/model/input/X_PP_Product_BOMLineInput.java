...
import org.bandahealth.idempiere.graphql.resolver.model.X_PP_Product_BOMLineResolver;
...
public class X_PP_Product_BOMLineInput extends MPPProductBOMLine implements I_PP_Product_BOMLineInput {
	 ...
	 *
	 * @param UU The PP_Product_BOMLine_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_PP_Product_BOMLineInput(@JsonProperty("UU") String UU) {
	...
	public void setComponentTypeInput(ForeignEntityInput ComponentType) {
		...
		if (ComponentType != null) {
			...
			if (!X_PP_Product_BOMLineResolver.COMPONENTTYPE_UUIDS_BY_VALUE.containsValue(ComponentType.getUU())) {
	...
	public void setIssueMethodInput(ForeignEntityInput IssueMethod) {
		...
		if (IssueMethod != null) {
			// Since an entity was passed, make sure it's in the list of acceptable values
			if (!X_PP_Product_BOMLineResolver.ISSUEMETHOD_UUIDS_BY_VALUE.containsValue(IssueMethod.getUU())) {
