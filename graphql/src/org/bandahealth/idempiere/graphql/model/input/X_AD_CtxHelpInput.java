...
import org.bandahealth.idempiere.graphql.resolver.model.X_AD_CtxHelpResolver;
...
public class X_AD_CtxHelpInput extends MCtxHelp implements I_AD_CtxHelpInput {
	 ...
	 *
	 * @param UU The AD_CtxHelp_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_AD_CtxHelpInput(@JsonProperty("UU") String UU) {
	...
	public void setCtxTypeInput(ForeignEntityInput CtxType) {
		...
		if (CtxType != null) {
			...
			if (!X_AD_CtxHelpResolver.CTXTYPE_UUIDS_BY_VALUE.containsValue(CtxType.getUU())) {
				throw new AdempiereException("The reference list UU of " + CtxType.getUU() +
						" is not in the list defined for the CtxType column");
			}
			// Now make sure it's in the DB
			MRefList_BH foreignEntity;
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(CtxType.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setCtxType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + CtxType.getUU());
			}
		} else {
