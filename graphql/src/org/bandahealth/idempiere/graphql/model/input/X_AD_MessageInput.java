...
import org.bandahealth.idempiere.graphql.resolver.model.X_AD_MessageResolver;
...
public class X_AD_MessageInput extends MMessage_BH implements I_AD_MessageInput {
	 ...
	 *
	 * @param UU The AD_Message_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_AD_MessageInput(@JsonProperty("UU") String UU) {
	...
	public void setMsgTypeInput(ForeignEntityInput MsgType) {
		...
		if (MsgType != null) {
			...
			if (!X_AD_MessageResolver.MSGTYPE_UUIDS_BY_VALUE.containsValue(MsgType.getUU())) {
				throw new AdempiereException("The reference list UU of " + MsgType.getUU() +
						" is not in the list defined for the MsgType column");
			}
			// Now make sure it's in the DB
			MRefList_BH foreignEntity;
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(MsgType.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setMsgType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + MsgType.getUU());
			}
		} else {
