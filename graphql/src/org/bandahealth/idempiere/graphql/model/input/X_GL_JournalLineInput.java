...
public class X_GL_JournalLineInput extends MJournalLine implements I_GL_JournalLineInput {
	 ...
	 */
	@JsonCreator
	public X_GL_JournalLineInput(@JsonProperty("ID") String ID) {
		super(null, ModelUtil.getModelResultSet(new MJournalLine(null, (ResultSet) null, null), null, Table_Name, ID),
				null);
		setID(ID);
	}
	...
	public void setAlias_ValidCombinationInput(ForeignEntityInput Alias_ValidCombination) {
		...
		if (Alias_ValidCombination != null &&
				(foreignEntity = new Query(getCtx(), "C_ValidCombination", "C_ValidCombination_UU=?", get_TrxName())
						.setParameters(Alias_ValidCombination.getID())
						.first()) != null && foreignEntity.get_ID() != 0) {
			super.setAlias_ValidCombination_ID(foreignEntity.get_ID());
		} else {
			super.setAlias_ValidCombination_ID(0);
		}
	}
