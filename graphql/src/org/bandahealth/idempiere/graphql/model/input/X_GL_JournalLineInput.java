...
public class X_GL_JournalLineInput extends MJournalLine implements I_GL_JournalLineInput {
	 ...
	 *
	 * @param UU The GL_JournalLine_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_GL_JournalLineInput(@JsonProperty("UU") String UU) {
		...
		if (Alias_ValidCombination != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_ValidCombination", "C_ValidCombination_UU=?", get_TrxName())
							.setParameters(Alias_ValidCombination.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setAlias_ValidCombination_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_ValidCombination with UU " + Alias_ValidCombination.getUU());
			}
		} else {
