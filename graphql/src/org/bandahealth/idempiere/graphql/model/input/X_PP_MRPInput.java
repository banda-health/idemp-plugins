...
public class X_PP_MRPInput extends X_PP_MRP implements I_PP_MRPInput {
	 ...
	 *
	 * @param UU The PP_MRP_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_PP_MRPInput(@JsonProperty("UU") String UU) {
	...
	public void setM_RequisitionInput(ForeignEntityInput M_Requisition) {
		...
		if (M_Requisition != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table M_Requisition with UU " + M_Requisition.getUU());
			}
		} else {
	...
	public void setM_RequisitionLineInput(ForeignEntityInput M_RequisitionLine) {
		...
		if (M_RequisitionLine != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table M_RequisitionLine with UU " + M_RequisitionLine.getUU());
			}
		} else {
	...
	public void setOrderTypeInput(ForeignEntityInput OrderType) {
		...
		if (OrderType != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(OrderType.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setOrderType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + OrderType.getUU());
			}
		} else {
	...
	public void setTypeMRPInput(ForeignEntityInput TypeMRP) {
		...
		if (TypeMRP != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(TypeMRP.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setTypeMRP(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + TypeMRP.getUU());
			}
		} else {
