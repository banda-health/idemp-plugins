...
import org.bandahealth.idempiere.graphql.resolver.model.X_PP_WF_Node_ProductResolver;
...
public class X_PP_WF_Node_ProductInput extends X_PP_WF_Node_Product implements I_PP_WF_Node_ProductInput {
	 ...
	 *
	 * @param UU The PP_WF_Node_Product_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_PP_WF_Node_ProductInput(@JsonProperty("UU") String UU) {
	...
	public void setConfigurationLevelInput(ForeignEntityInput ConfigurationLevel) {
		...
		if (ConfigurationLevel != null) {
			...
			if (!X_PP_WF_Node_ProductResolver.CONFIGURATIONLEVEL_UUIDS_BY_VALUE.containsValue(ConfigurationLevel.getUU())) {
				throw new AdempiereException("The reference list UU of " + ConfigurationLevel.getUU() +
						" is not in the list defined for the ConfigurationLevel column");
			}
			// Now make sure it's in the DB
			MRefList_BH foreignEntity;
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(ConfigurationLevel.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setConfigurationLevel(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + ConfigurationLevel.getUU());
			}
		} else {
