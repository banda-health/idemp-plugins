...
public class X_C_OrderLandedCostInput extends MOrderLandedCost implements I_C_OrderLandedCostInput {
	 ...
	 */
	@JsonCreator
	public X_C_OrderLandedCostInput(@JsonProperty("UUID") String UUID) {
		super(Env.getCtx(), ModelUtil.getModelResultSet(new MOrderLandedCost(null, (ResultSet) null, null),
	...
	public void setLandedCostDistributionInput(I_AD_Ref_ListInput LandedCostDistribution) {
		...
		if (LandedCostDistribution != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UUID " + LandedCostDistribution.getUUID());
			}
		} else {
