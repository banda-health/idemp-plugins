...
public class X_M_BP_PriceInput extends X_M_BP_Price implements I_M_BP_PriceInput {
	 ...
	 *
	 * @param UU The M_BP_Price_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_M_BP_PriceInput(@JsonProperty("UU") String UU) {
		...
		if (PriceOverrideType != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(PriceOverrideType.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setPriceOverrideType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + PriceOverrideType.getUU());
			}
		} else {
