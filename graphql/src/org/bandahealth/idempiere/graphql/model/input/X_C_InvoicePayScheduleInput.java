...
public class X_C_InvoicePayScheduleInput extends MInvoicePaySchedule implements I_C_InvoicePayScheduleInput {
	 ...
	 *
	 * @param UU The C_InvoicePaySchedule_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_C_InvoicePayScheduleInput(@JsonProperty("UU") String UU) {
		...
		if (C_PaySchedule != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_PaySchedule", "C_PaySchedule_UU=?", get_TrxName())
							.setParameters(C_PaySchedule.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setC_PaySchedule_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_PaySchedule with UU " + C_PaySchedule.getUU());
			}
		} else {
