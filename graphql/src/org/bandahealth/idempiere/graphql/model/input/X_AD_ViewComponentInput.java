...
public class X_AD_ViewComponentInput extends MViewComponent implements I_AD_ViewComponentInput {
	 ...
	 *
	 * @param UU The AD_ViewComponent_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_AD_ViewComponentInput(@JsonProperty("UU") String UU) {
	...
	public void setReferenced_TableInput(ForeignEntityInput Referenced_Table) {
		...
		if (Referenced_Table != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "AD_Table", "AD_Table_UU=?", get_TrxName())
							.setParameters(Referenced_Table.getUU()).first()) != null && foreignEntity.get_ID() >= 1) {
				this.setReferenced_Table_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table AD_Table with UU " + Referenced_Table.getUU());
			}
		} else {
