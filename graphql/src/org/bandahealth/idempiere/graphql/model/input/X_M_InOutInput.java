...
public class X_M_InOutInput extends MInOut_BH implements I_M_InOutInput {
	 ...
	 *
	 * @param UU The M_InOut_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_M_InOutInput(@JsonProperty("UU") String UU) {
	...
	public void setReturnBPartnerInput(ForeignEntityInput ReturnBPartner) {
		...
		if (ReturnBPartner != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_BPartner", "C_BPartner_UU=?", get_TrxName())
							.setParameters(ReturnBPartner.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
	...
	public void setReturnLocationInput(ForeignEntityInput ReturnLocation) {
		...
		if (ReturnLocation != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_BPartner_Location", "C_BPartner_Location_UU=?", get_TrxName())
							.setParameters(ReturnLocation.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
	...
	public void setReturnUserInput(ForeignEntityInput ReturnUser) {
		...
		if (ReturnUser != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "AD_User", "AD_User_UU=?", get_TrxName())
							.setParameters(ReturnUser.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
	...
	public void setReversalInput(ForeignEntityInput Reversal) {
		...
		if (Reversal != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table M_InOut with UU " + Reversal.getUU());
			}
		} else {
