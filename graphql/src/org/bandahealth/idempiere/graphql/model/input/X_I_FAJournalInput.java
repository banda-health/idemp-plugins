...
import org.bandahealth.idempiere.graphql.resolver.model.X_I_FAJournalResolver;
...
public class X_I_FAJournalInput extends MXIFAJournal implements I_I_FAJournalInput {
	 ...
	 *
	 * @param UU The I_FAJournal_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_I_FAJournalInput(@JsonProperty("UU") String UU) {
		...
		if (C_ValidCombination != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_ValidCombination", "C_ValidCombination_UU=?", get_TrxName())
							.setParameters(C_ValidCombination.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
	...
	public void setCurrencyRateTypeInput(ForeignEntityInput CurrencyRateType) {
		...
		if (CurrencyRateType != null) {
			...
			if (!X_I_FAJournalResolver.CURRENCYRATETYPE_UUIDS_BY_VALUE.containsValue(CurrencyRateType.getUU())) {
				throw new AdempiereException("The reference list UU of " + CurrencyRateType.getUU() +
						" is not in the list defined for the CurrencyRateType column");
			}
			// Now make sure it's in the DB
			MRefList_BH foreignEntity;
			if ((foreignEntity =
					new Query(getCtx(), MRefList_BH.Table_Name, MRefList_BH.COLUMNNAME_AD_Ref_List_UU + "=?", get_TrxName())
							.setParameters(CurrencyRateType.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setCurrencyRateType(foreignEntity.getValue());
			} else {
				throw new AdempiereException(
						"Could not find entity in table " + MRefList_BH.Table_Name + " with UU " + CurrencyRateType.getUU());
			}
		} else {
			this.setCurrencyRateType(null);
		}
	}
		...
		if (GL_Journal != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "GL_Journal", "GL_Journal_UU=?", get_TrxName())
							.setParameters(GL_Journal.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
		...
		if (GL_JournalLine != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "GL_JournalLine", "GL_JournalLine_UU=?", get_TrxName())
							.setParameters(GL_JournalLine.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setGL_JournalLine_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table GL_JournalLine with UU " + GL_JournalLine.getUU());
			}
		} else {
	...
	public void setPostingTypeInput(ForeignEntityInput PostingType) {
		...
		if (PostingType != null) {
			// Since an entity was passed, make sure it's in the list of acceptable values
			if (!X_I_FAJournalResolver.POSTINGTYPE_UUIDS_BY_VALUE.containsValue(PostingType.getUU())) {
