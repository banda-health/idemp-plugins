...
import org.bandahealth.idempiere.graphql.resolver.model.X_PP_Product_BOMResolver;
...
public class X_PP_Product_BOMInput extends MPPProductBOM implements I_PP_Product_BOMInput {
	 ...
	 *
	 * @param UU The PP_Product_BOM_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_PP_Product_BOMInput(@JsonProperty("UU") String UU) {
	...
	public void setBOMTypeInput(ForeignEntityInput BOMType) {
		...
		if (BOMType != null) {
			...
			if (!X_PP_Product_BOMResolver.BOMTYPE_UUIDS_BY_VALUE.containsValue(BOMType.getUU())) {
	...
	public void setBOMUseInput(ForeignEntityInput BOMUse) {
		...
		if (BOMUse != null) {
			// Since an entity was passed, make sure it's in the list of acceptable values
			if (!X_PP_Product_BOMResolver.BOMUSE_UUIDS_BY_VALUE.containsValue(BOMUse.getUU())) {
