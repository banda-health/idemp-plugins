...
import org.bandahealth.idempiere.graphql.resolver.model.X_C_ProjectIssueResolver;
...
public class X_C_ProjectIssueInput extends MProjectIssue implements I_C_ProjectIssueInput {
	 ...
	 *
	 * @param UU The C_ProjectIssue_UU to fetch this entity from the DB
	 */
	@JsonCreator
	public X_C_ProjectIssueInput(@JsonProperty("UU") String UU) {
	...
	public void setDocActionInput(ForeignEntityInput DocAction) {
		...
		if (DocAction != null) {
			...
			if (!X_C_ProjectIssueResolver.DOCACTION_UUIDS_BY_VALUE.containsValue(DocAction.getUU())) {
	...
	public void setDocStatusInput(ForeignEntityInput DocStatus) {
		...
		if (DocStatus != null) {
			// Since an entity was passed, make sure it's in the list of acceptable values
			if (!X_C_ProjectIssueResolver.DOCSTATUS_UUIDS_BY_VALUE.containsValue(DocStatus.getUU())) {
	...
	public void setReversalInput(ForeignEntityInput Reversal) {
		...
		if (Reversal != null) {
			...
			if ((foreignEntity =
				...
				throw new AdempiereException(
						"Could not find entity in table C_ProjectIssue with UU " + Reversal.getUU());
			}
		} else {
		...
		if (S_TimeExpenseLine != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "S_TimeExpenseLine", "S_TimeExpenseLine_UU=?", get_TrxName())
							.setParameters(S_TimeExpenseLine.getUU()).first()) != null && foreignEntity.get_ID() >= 0) {
				this.setS_TimeExpenseLine_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table S_TimeExpenseLine with UU " + S_TimeExpenseLine.getUU());
			}
		} else {
