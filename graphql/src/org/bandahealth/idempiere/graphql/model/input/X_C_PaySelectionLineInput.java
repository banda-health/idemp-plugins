...
public class X_C_PaySelectionLineInput extends MPaySelectionLine implements I_C_PaySelectionLineInput {
	 ...
	 */
	@JsonCreator
	public X_C_PaySelectionLineInput(@JsonProperty("UUID") String UUID) {
		super(Env.getCtx(), ModelUtil.getModelResultSet(new MPaySelectionLine(null, (ResultSet) null, null),
		...
		if (C_PaySelectionCheck != null) {
			...
			if ((foreignEntity =
					new Query(getCtx(), "C_PaySelectionCheck", "C_PaySelectionCheck_UU=?", get_TrxName())
							.setParameters(C_PaySelectionCheck.getUUID()).first()) != null && foreignEntity.get_ID() != 0) {
				this.setC_PaySelectionCheck_ID(foreignEntity.get_ID());
			} else {
				throw new AdempiereException(
						"Could not find entity in table C_PaySelectionCheck with UUID " + C_PaySelectionCheck.getUUID());
			}
		} else {
