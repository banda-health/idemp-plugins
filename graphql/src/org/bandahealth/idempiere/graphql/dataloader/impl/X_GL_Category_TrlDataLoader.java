...
public class X_GL_Category_TrlDataLoader extends PODataLoader<PO> {
	...
	protected String getTableName() {
		return MGLCategory.Table_Name + "_Trl";
	}
	...
	private MappedBatchLoaderWithContext<Integer, PO> getByIdAndLanguageBatchLoader() {
		return (keys, batchLoaderEnvironment) -> CompletableFuture.supplyAsync(() -> {
			...
			List<PO> translations = new Query(batchLoaderEnvironment.getContext(), getTableName(),
					MGLCategory.COLUMNNAME_GL_Category_ID + " IN (" + whereClause + ") AND AD_Language = ?", null).setParameters(
					parameters).list();
			return translations.stream().collect(
					Collectors.toMap(translation -> translation.get_ValueAsInt(MGLCategory.COLUMNNAME_GL_Category_ID),
