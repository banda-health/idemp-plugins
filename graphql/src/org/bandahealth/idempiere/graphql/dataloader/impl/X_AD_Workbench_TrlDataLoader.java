...
/**
 * Data Loader for AD_Workbench_Trl - DO NOT CHANGE
 *
...
public class X_AD_Workbench_TrlDataLoader extends PODataLoader<PO> {
	...
	protected String getTableName() {
		return X_AD_Workbench.Table_Name + "_Trl";
	}
	...
	private MappedBatchLoaderWithContext<Integer, PO> getByIdAndLanguageBatchLoader() {
		return (keys, batchLoaderEnvironment) -> CompletableFuture.supplyAsync(() -> {
			...
			List<PO> translations = new Query(batchLoaderEnvironment.getContext(), getTableName(),
					X_AD_Workbench.COLUMNNAME_AD_Workbench_ID + " IN (" + whereClause + ") AND AD_Language = ?", null).setParameters(
					parameters).list();
			return translations.stream().collect(
					Collectors.toMap(translation -> translation.get_ValueAsInt(X_AD_Workbench.COLUMNNAME_AD_Workbench_ID),
