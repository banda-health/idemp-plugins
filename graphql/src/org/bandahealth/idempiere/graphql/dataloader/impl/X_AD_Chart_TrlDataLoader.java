...
public class X_AD_Chart_TrlDataLoader extends PODataLoader<PO> {
	public static String AD_Chart_Trl_BY_ID_DATA_LOADER = "AD_Chart_TrlByIdDataLoader";
	public static String AD_Chart_Trl_BY_UUID_DATA_LOADER = "AD_Chart_TrlByUuidDataLoader";

	@Override
	protected String getTableName() {
		return MChart.Table_Name + "_Trl";
	}
	...
	protected String getByUuidDataLoaderName() {
		return AD_Chart_Trl_BY_UUID_DATA_LOADER;
	}

	@Override
	public void register(DataLoaderRegistry registry, Properties idempiereContext) {
		super.register(registry, idempiereContext);
		registry.register(AD_Chart_Trl_BY_ID_DATA_LOADER,
	...
	private MappedBatchLoaderWithContext<Integer, PO> getByIdAndLanguageBatchLoader() {
		return (keys, batchLoaderEnvironment) -> CompletableFuture.supplyAsync(() -> {
			...
			List<PO> translations = new Query(batchLoaderEnvironment.getContext(), getTableName(),
					MChart.COLUMNNAME_AD_Chart_ID + " IN (" + whereClause + ") AND AD_Language = ?", null).setParameters(
					parameters).list();
			return translations.stream().collect(
					Collectors.toMap(translation -> translation.get_ValueAsInt(MChart.COLUMNNAME_AD_Chart_ID),
