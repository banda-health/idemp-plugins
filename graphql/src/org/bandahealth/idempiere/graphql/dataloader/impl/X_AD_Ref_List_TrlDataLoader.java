...
public class X_AD_Ref_List_TrlDataLoader extends PODataLoader<PO> {
	public static String AD_Ref_List_Trl_BY_ID_DATA_LOADER = "AD_Ref_List_TrlByIdDataLoader";
	public static String AD_Ref_List_Trl_BY_UUID_DATA_LOADER = "AD_Ref_List_TrlByUuidDataLoader";

	@Override
	protected String getTableName() {
		return MRefList_BH.Table_Name + "_Trl";
	}
	...
	protected String getByUuidDataLoaderName() {
		return AD_Ref_List_Trl_BY_UUID_DATA_LOADER;
	}

	@Override
	public void register(DataLoaderRegistry registry, Properties idempiereContext) {
		super.register(registry, idempiereContext);
		registry.register(AD_Ref_List_Trl_BY_ID_DATA_LOADER,
	...
	private MappedBatchLoaderWithContext<Integer, PO> getByIdAndLanguageBatchLoader() {
		return (keys, batchLoaderEnvironment) -> CompletableFuture.supplyAsync(() -> {
			...
			List<PO> translations = new Query(batchLoaderEnvironment.getContext(), getTableName(),
					MRefList_BH.COLUMNNAME_AD_Ref_List_ID + " IN (" + whereClause + ") AND AD_Language = ?", null).setParameters(
					parameters).list();
			return translations.stream().collect(
					Collectors.toMap(translation -> translation.get_ValueAsInt(MRefList_BH.COLUMNNAME_AD_Ref_List_ID),
