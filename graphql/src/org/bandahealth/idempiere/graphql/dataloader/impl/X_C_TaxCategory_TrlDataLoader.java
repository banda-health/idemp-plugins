...
public class X_C_TaxCategory_TrlDataLoader extends PODataLoader<PO> {
	...
	public static String DATALOADER_C_TaxCategory_Trl_BY_UUID = "C_TaxCategory_TrlByUuidDataLoader";

	@Override
	protected String getTableName() {
		return MTaxCategory.Table_Name + "_Trl";
	}
	...
	protected String getByUuidDataLoaderName() {
		return DATALOADER_C_TaxCategory_Trl_BY_UUID;
	}

	@Override
	public void register(DataLoaderRegistry registry, Properties idempiereContext) {
		super.register(registry, idempiereContext);
		registry.register(DATALOADER_C_TaxCategory_Trl_BY_ID,
	...
	private MappedBatchLoaderWithContext<Integer, PO> getByIdAndLanguageBatchLoader() {
		return (keys, batchLoaderEnvironment) -> CompletableFuture.supplyAsync(() -> {
			...
			List<PO> translations = new Query(batchLoaderEnvironment.getContext(), getTableName(),
					MTaxCategory.COLUMNNAME_C_TaxCategory_ID + " IN (" + whereClause + ") AND AD_Language = ?", null).setParameters(
					parameters).list();
			return translations.stream().collect(
					Collectors.toMap(translation -> translation.get_ValueAsInt(MTaxCategory.COLUMNNAME_C_TaxCategory_ID),
