...
public class X_PA_DashboardContent_TrlDataLoader extends PODataLoader<PO> {
	...
	protected String getTableName() {
		return MDashboardContent.Table_Name + "_Trl";
	}
	...
	private MappedBatchLoaderWithContext<Integer, PO> getByIdAndLanguageBatchLoader() {
		return (keys, batchLoaderEnvironment) -> CompletableFuture.supplyAsync(() -> {
			...
			List<PO> translations = new Query(batchLoaderEnvironment.getContext(), getTableName(),
					MDashboardContent.COLUMNNAME_PA_DashboardContent_ID + " IN (" + whereClause + ") AND AD_Language = ?", null).setParameters(
					parameters).list();
			return translations.stream().collect(
					Collectors.toMap(translation -> translation.get_ValueAsInt(MDashboardContent.COLUMNNAME_PA_DashboardContent_ID),
