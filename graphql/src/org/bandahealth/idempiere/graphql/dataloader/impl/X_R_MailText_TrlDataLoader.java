...
/**
 * Data Loader for R_MailText_Trl - DO NOT CHANGE
 *
...
public class X_R_MailText_TrlDataLoader extends PODataLoader<PO> {
	...
	protected String getTableName() {
		return MMailText.Table_Name + "_Trl";
	}
	...
	private MappedBatchLoaderWithContext<Integer, PO> getByIdAndLanguageBatchLoader() {
		return (keys, batchLoaderEnvironment) -> CompletableFuture.supplyAsync(() -> {
			...
			List<PO> translations = new Query(batchLoaderEnvironment.getContext(), getTableName(),
					MMailText.COLUMNNAME_R_MailText_ID + " IN (" + whereClause + ") AND AD_Language = ?", null).setParameters(
					parameters).list();
			return translations.stream().collect(
					Collectors.toMap(translation -> translation.get_ValueAsInt(MMailText.COLUMNNAME_R_MailText_ID),
