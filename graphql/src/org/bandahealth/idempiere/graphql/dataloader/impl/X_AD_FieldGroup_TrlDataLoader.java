...
public class X_AD_FieldGroup_TrlDataLoader extends PODataLoader<PO> {
	...
	public static String DATALOADER_AD_FieldGroup_Trl_BY_UUID = "AD_FieldGroup_TrlByUuidDataLoader";

	@Override
	protected String getTableName() {
		return MFieldGroup_BH.Table_Name + "_Trl";
	}
	...
	protected String getByUuidDataLoaderName() {
		return DATALOADER_AD_FieldGroup_Trl_BY_UUID;
	}

	@Override
	public void register(DataLoaderRegistry registry, Properties idempiereContext) {
		super.register(registry, idempiereContext);
		registry.register(DATALOADER_AD_FieldGroup_Trl_BY_ID,
	...
	private MappedBatchLoaderWithContext<Integer, PO> getByIdAndLanguageBatchLoader() {
		return (keys, batchLoaderEnvironment) -> CompletableFuture.supplyAsync(() -> {
			...
			List<PO> translations = new Query(batchLoaderEnvironment.getContext(), getTableName(),
					MFieldGroup_BH.COLUMNNAME_AD_FieldGroup_ID + " IN (" + whereClause + ") AND AD_Language = ?", null).setParameters(
					parameters).list();
			return translations.stream().collect(
					Collectors.toMap(translation -> translation.get_ValueAsInt(MFieldGroup_BH.COLUMNNAME_AD_FieldGroup_ID),
