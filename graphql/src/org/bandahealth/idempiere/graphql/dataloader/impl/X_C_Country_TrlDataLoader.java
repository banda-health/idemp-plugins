...
public class X_C_Country_TrlDataLoader extends PODataLoader<PO> {
	public static String C_Country_Trl_BY_ID_DATA_LOADER = "C_Country_TrlByIdDataLoader";
	public static String C_Country_Trl_BY_UUID_DATA_LOADER = "C_Country_TrlByUuidDataLoader";

	@Override
	protected String getTableName() {
		return MCountry.Table_Name + "_Trl";
	}
	...
	protected String getByUuidDataLoaderName() {
		return C_Country_Trl_BY_UUID_DATA_LOADER;
	}

	@Override
	public void register(DataLoaderRegistry registry, Properties idempiereContext) {
		super.register(registry, idempiereContext);
		registry.register(C_Country_Trl_BY_ID_DATA_LOADER,
	...
	private MappedBatchLoaderWithContext<Integer, PO> getByIdAndLanguageBatchLoader() {
		return (keys, batchLoaderEnvironment) -> CompletableFuture.supplyAsync(() -> {
			...
			List<PO> translations = new Query(batchLoaderEnvironment.getContext(), getTableName(),
					MCountry.COLUMNNAME_C_Country_ID + " IN (" + whereClause + ") AND AD_Language = ?", null).setParameters(
					parameters).list();
			return translations.stream().collect(
					Collectors.toMap(translation -> translation.get_ValueAsInt(MCountry.COLUMNNAME_C_Country_ID),
