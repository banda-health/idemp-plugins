...
public class PaymentRepository extends BaseRepository<MPayment_BH, PaymentInput> {
	...
	public CompletableFuture<Map<String, List<MPayment_BH>>> getByOrderIds(Set<String> orderIdKeys,
		...
		List<MPayment_BH> payments = getBaseQuery(idempiereContext, MPayment_BH.COLUMNNAME_C_Order_ID +
						" IN (" + whereCondition + ") OR " + MPayment_BH.COLUMNNAME_C_Order_ID + " IN (" + whereCondition + ")",
				parameters).setOnlyActiveRecords(true).list();
		return CompletableFuture.supplyAsync(() ->
				payments.stream().collect(Collectors.groupingBy(payment ->
						ModelUtil.getModelKey(modelName, payment.getC_Order_ID()))));
	}
	...
	public Connection<MPayment_BH> getServiceDebtPayments(String filterJson, String sort, PagingInfo pagingInfo,
		...
		return get(filterJson, sort, pagingInfo, MPayment_BH.COLUMNNAME_BH_Visit_ID + " IS NULL", null,
				join, environment);
	}

	public void deleteByOrder(int orderId, List<String> paymentUuidsToKeep, Properties idempiereContext) {
		String whereClause = "(" + MPayment_BH.COLUMNNAME_C_Order_ID + "=?)";
