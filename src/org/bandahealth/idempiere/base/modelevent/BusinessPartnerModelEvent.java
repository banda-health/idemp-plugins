...
//		businessPartner.setBPGroup(group);
		...
		if (businessPartner.isCustomer()) {
			...
			I_AD_Ref_List invoiceRule = QueryUtil.queryTableByOrgAndClient(clientId, orgId, Env.getCtx(), I_AD_Ref_List.Table_Name,
			...
			I_AD_Reference paymentRuleReference = QueryUtil.queryTableByOrgAndClient(clientId, orgId, Env.getCtx(), I_AD_Reference.Table_Name,
					"name = '_Payment Rule'", null);
			I_AD_Ref_List paymentRule = QueryUtil.queryTableByOrgAndClient(clientId, orgId, Env.getCtx(), I_AD_Ref_List.Table_Name,
			...
			MPaymentTerm paymentTerm = QueryUtil.queryTableByOrgAndClient(clientId, orgId, Env.getCtx(), MPaymentTerm.Table_Name,
					"name = 'Immediate'", null);
			businessPartner.setC_PaymentTerm_ID(paymentTerm.getC_PaymentTerm_ID());
			
			// Set the price list
			MPriceList priceList = QueryUtil.queryTableByOrgAndClient(clientId, orgId, Env.getCtx(), MPriceList.Table_Name,
		...
		if (businessPartner.isVendor()) {
			// Set the payment rule
			I_AD_Reference vendorPaymentRuleReference = QueryUtil.queryTableByOrgAndClient(clientId, orgId, Env.getCtx(), I_AD_Reference.Table_Name,
					"name = '_Payment Rule'", null);
			I_AD_Ref_List vendorPaymentRule = QueryUtil.queryTableByOrgAndClient(clientId, orgId, Env.getCtx(), I_AD_Ref_List.Table_Name,
			...
			MPaymentTerm purchasePaymentTerm = QueryUtil.queryTableByOrgAndClient(clientId, orgId, Env.getCtx(), MPaymentTerm.Table_Name,
					"name = 'Immediate'", null);
			businessPartner.setPO_PaymentTerm_ID(purchasePaymentTerm.getC_PaymentTerm_ID());
			
			// Set the purchase price list
			MPriceList purchasePriceList = QueryUtil.queryTableByOrgAndClient(clientId, orgId, Env.getCtx(), MPriceList.Table_Name,
	...
	private void initializeDependentEntities(MBPartner businessPartner) {
		...
		if (businessPartner.getLocations(true).length == 0) {
			...
			MCountry country = (new Query(Env.getCtx(), I_C_Country.Table_Name, "countrycode = '" + IBHConfig.DEFAULT_LOCATION_COUNTRY_CODE
					+ "'", null)).first();
