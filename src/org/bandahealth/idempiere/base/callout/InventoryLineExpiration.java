...
public class InventoryLineExpiration implements IColumnCallout {
		...
		if (mField.getColumnName().equalsIgnoreCase(MProduct_BH.COLUMNNAME_M_Product_ID)) {
			if (value != null) {
				...
				if (!product.hasExpiration()) {
					mTab.getField(MInventoryLine_BH.COLUMNNAME_BH_Expiration).setDisplayed(false);
				} else {
					mTab.getField(MInventoryLine_BH.COLUMNNAME_BH_Expiration).setDisplayed(true);
				}
			}
		}
