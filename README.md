...
**Local dev (recommended):** `./dev.sh test` ensures iDempiere is running, then runs **GraphQL Jest only** by default. Use `./dev.sh test --all` for the full suite, `./dev.sh test graphql` for SOAP + Jest, or `./dev.sh test -- visitReceiptReport.test.ts` for a single Jest file. Run `./dev.sh build` and `./dev.sh migrate` separately when needed.
...
### GraphQL API Tests
These are run via Jest to connect to the server and test the endpoints. It is a TypeScript project that needs the types the API returns. These are automatically generated by building the `graphql` plugin, so please do that first.
