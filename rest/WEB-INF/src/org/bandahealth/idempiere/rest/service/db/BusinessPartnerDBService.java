...
public class BusinessPartnerDBService extends BaseDBService<BusinessPartner, MBPartner_BH> {
	...
	public BusinessPartner saveEntity(BusinessPartner entity) {
		...
		if (StringUtil.isNotNullAndEmpty(entity.getAddress())) {
			...
			MBPartnerLocation businessPartnerLocation = new Query(Env.getCtx(), MBPartnerLocation.Table_Name,
					MBPartnerLocation.COLUMNNAME_C_Location_ID + "=? AND " + MBPartnerLocation.COLUMNNAME_C_BPartner_ID + "=?",
					null).setParameters(location.get_ID(), businessPartner.getC_BPartner_ID()).setClient_ID().first();
			if (businessPartnerLocation == null) {
				businessPartnerLocation = new MBPartnerLocation(Env.getCtx(), 0, null);
			}
	...
	public List<BusinessPartner> transformData(List<MBPartner_BH> dbModels) {
		...
		// Batch calls
		Map<Integer, MBPGroup_BH> businessPartnerGroupsById =
				businessPartnerGroupDBService.getByIds(businessPartnerGroupIds);
		...
		Map<Integer, List<MBHPayerInfoField>> payerInformationFieldsByPayerId = payerInformationFieldDBService
				.getGroupsByIds(MBHPayerInfoField::getBH_Payer_ID, MBHPayerInfoField.COLUMNNAME_BH_Payer_ID,
						businessPartnerIds);
		...
		Set<Integer> payerInformationFieldIds = payerInformationFieldsByPayerId.values().stream()
				.flatMap(paymentInformationFieldList -> paymentInformationFieldList.stream().map(MBHPayerInfoField::get_ID))
				.collect(Collectors.toSet());
		Map<Integer, List<MBHPayerInfoFieldValue>> payerInformationFieldValueListByPayerInformationFieldId =
				payerInformationFieldValueDBService
						.getGroupsByIds(MBHPayerInfoFieldValue::getBH_Payer_Info_Field_ID, MBHPayerInfoFieldValue
										.COLUMNNAME_BH_Payer_Info_Field_ID,
				...
				referenceListDBService.getTypes(MReference_BH.PAYER_INFORMATION_FIELD_DATA_TYPE_AD_REFERENCE_UU,
								payerInformationFieldsByPayerId.values().stream().flatMap(
										payemntInformationFieldList -> payemntInformationFieldList.stream()
												.map(MBHPayerInfoField::getBH_PayerInfoFieldDataType)).collect(Collectors.toSet())).stream()
		...
		return dbModels.stream().map(businessPartner -> {
			...
			if (businessPartnerGroupsById.containsKey(businessPartner.getC_BP_Group_ID())) {
				businessPartnerToReturn.setBusinessPartnerGroup(
						new BusinessPartnerGroup(businessPartnerGroupsById.get(businessPartner.getC_BP_Group_ID())));
			}
